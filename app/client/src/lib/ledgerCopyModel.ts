...
export function ledgerOverviewCopy() {
  return {
    ...
    memoryContractTitle: "Memory Reuse Read",
    memoryContractBadge: "retrieval off",
    memoryContractAria: "Ledger memory reuse read",
    memoryContractRouteLabel: "Route",
    memoryContractReviewLabel: "Review",
    memoryContractGateLabel: "Gate",
    memoryContractNextLabel: "Next",
    memoryContractRouteValue: (normalRoute: string, archiveRoute: string) => `${normalRoute} / ${archiveRoute}`,
    memoryContractReviewValue: (pending: number, validated: number) => `${pending} pending, ${validated} Oak validated`,
