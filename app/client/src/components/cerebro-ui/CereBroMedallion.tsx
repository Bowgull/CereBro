...
export function CereBroMedallion({ active = false, imageSrc, label, fallback, variant = "framed", className = "", style, ...props }: CereBroMedallionProps) {
  ...
  return (
    <button
      ...
      {...props}
    >
      {!rendersAssetChrome && <span className="absolute inset-1 rounded-full" style={{ border: `1px solid ${B.line.brassSoft}` }} aria-hidden="true" />}
      {imageSrc ? <img src={imageSrc} alt="" className={rendersAssetChrome ? "absolute inset-0 h-full w-full rounded-full object-contain" : "relative h-7 w-7 rounded object-contain"} draggable={false} /> : <span className="relative">{fallback ?? <Plus size={18} strokeWidth={1.7} aria-hidden="true" />}</span>}
