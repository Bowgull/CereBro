...
describe("installed desktop app QA script", () => {
  it("tests the installed app instead of the dev server", async () => {
    ...
    const mockupManifest = JSON.parse(await readFile(resolve(appRoot, "../mockups/approved/manifest.json"), "utf8")) as {
      lockedPrimaryBrowserHomeReference: string;
      lockedPrimaryBrowserHomeSha256: string;
    };
    const browserHomeLockSource = await readFile(resolve(appRoot, "../mockups/approved/BROWSER_HOME_1TO1_LOCK.md"), "utf8");
    ...
    expect(mockupManifest.lockedPrimaryBrowserHomeReference).toBe("browser-home-symmetric-rails-target-v1.png");
    ...
    expect(browserHomeDiffSource).not.toContain("browser-loaded-website-target-v1.png");
