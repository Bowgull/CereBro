...
async function ensureSchema(client: Client): Promise<void> {
  await client.batch(
    [
      ...
      `CREATE TABLE IF NOT EXISTS surfer_browser_adapter_receipts (
         id INTEGER PRIMARY KEY AUTOINCREMENT,
         ...
         approval_id INTEGER REFERENCES approvals(id) ON DELETE SET NULL,
         status TEXT NOT NULL DEFAULT 'approval_preview',
         owner_agent TEXT NOT NULL DEFAULT 'surfer',
         can_run INTEGER NOT NULL DEFAULT 0,
         install_configured INTEGER NOT NULL DEFAULT 0,
         opens_browser INTEGER NOT NULL DEFAULT 0,
         writes_memory INTEGER NOT NULL DEFAULT 0,
         writes_external_systems INTEGER NOT NULL DEFAULT 0,
         allowed_actions_json TEXT NOT NULL,
         blocked_actions_json TEXT NOT NULL,
         no_action_taken TEXT NOT NULL,
         created_at INTEGER NOT NULL DEFAULT (unixepoch()),
         updated_at INTEGER NOT NULL DEFAULT (unixepoch())
       )`,
      `CREATE INDEX IF NOT EXISTS idx_surfer_browser_adapter_receipts_adapter ON surfer_browser_adapter_receipts(adapter)`,
      `CREATE INDEX IF NOT EXISTS idx_surfer_browser_adapter_receipts_created ON surfer_browser_adapter_receipts(created_at DESC)`,
      `CREATE INDEX IF NOT EXISTS idx_surfer_browser_adapter_receipts_approval ON surfer_browser_adapter_receipts(approval_id)`,
      ...
      `CREATE INDEX IF NOT EXISTS idx_browser_bookmarks_created ON browser_bookmarks(created_at DESC)`,
      `CREATE INDEX IF NOT EXISTS idx_browser_bookmarks_target ON browser_bookmarks(target_url)`,
      `CREATE TABLE IF NOT EXISTS browser_runner_audit_records (
         id INTEGER PRIMARY KEY AUTOINCREMENT,
         proposal_id INTEGER REFERENCES browser_action_proposals(id) ON DELETE SET NULL,
         runner_state TEXT NOT NULL,
         can_open_page INTEGER NOT NULL DEFAULT 0,
         can_execute INTEGER NOT NULL DEFAULT 0,
         receipt_body TEXT NOT NULL,
         no_action_taken TEXT NOT NULL,
         created_at INTEGER NOT NULL DEFAULT (unixepoch())
       )`,
      `CREATE INDEX IF NOT EXISTS idx_browser_runner_audit_records_proposal ON browser_runner_audit_records(proposal_id)`,
      `CREATE INDEX IF NOT EXISTS idx_browser_runner_audit_records_created ON browser_runner_audit_records(created_at DESC)`,
