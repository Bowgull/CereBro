...
- **MCP Server + CLI track ⏳ NEW** — expose CereBro brain as an MCP server (Claude Code sessions call in directly) and a `cerebro` CLI (headless, Bash-tool-friendly). Full design in `CereBro_V1_MCP_CLI_Design.md`. Implementation: Session 1 = MCP server (`app/server/mcp/` + `.mcp.json`), Session 2 = CLI (`cli/` package, Commander.js, `createCallerFactory`), Session 3 = hardening. No new DB schema needed — tRPC routers are 80% of the work already done.
