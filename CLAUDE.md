...
- `app/` — the running client+server. Forked from `thousandsky2024/claude-pixel-agent-web` (MIT, "Claude Dungeon"). React 19 + Tailwind 4 + Phaser 3 + Express + tRPC + Drizzle + WebSocket.
  ...
  - `app/client/src/components/KeepScene.tsx` — **the live Phaser scene**. Side-cutaway castle: 3 floors stacked, 10 chambers laid out left-to-right. Stone walls/floor/banners/torches from 0x72 DungeonTilesetII. Cortana hub centerpiece with violet dais + floating user-orb. Cortana sprite has vertical bob tween (proof of motion). Piccolo gets the fountain. Other agents placed static for now.
...
- Keep UI parallel track ⏳ side-cutaway Phaser scene shipped in session 5 (KeepScene.tsx), Cortana motion proven. Deferred to next pass: stairs/ladders between floors (0x72 II `floor_stairs`/`floor_ladder` tiles), per-chamber floor variety (`floor_1`–`floor_8`), universal furniture combos (chests/crates/columns/flasks/banners) so rooms stop reading as identical, idle anim for the other 9 agents, ambient effects (torch flicker 8-frame, fountain ripple 3-frame), active/dormant state visualization, Cortana-faces-orb dynamic flip. Phase 3.5: custom-pixeled signature props (anvil, bookshelf, telescope, map table, scales, hourglass) to give each chamber real character signature. Tested 2026-05-05 that Kenney 1-bit pack cannot mix with 0x72 even after recolor (line weight + shading mismatch); Kenney 1-bit reserved for UI-only icons later. Test artefact: `mockups/style-mix-test.png`.
