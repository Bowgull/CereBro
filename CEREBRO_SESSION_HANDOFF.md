...
### What Changed
- Security Gate no longer loads Workbench source link options on panel open.
- Source linking now sits behind a `Source Link` disclosure that says
  `open to read` while closed.
- Opening `Source Link` reads local source links with a 30 second stale cache.
- Browser-checked the closed and opened Security Gate source-link states.
...
- `pnpm -C app check` passed.
- Browser proof with Playwright CLI against `http://localhost:3000/`:
  Basement -> Security shows `Source Link open to read`; opening it shows local
  source count and the source link selector.
- `git diff --check` passed.
- `curl -I --max-time 5 http://localhost:3000/` returned `HTTP/1.1 200 OK`.

### Cleanliness Read
- Current slice: Security Gate read reduction.
- No backend code, schema, task execution, approval decision, command
  execution, browser action from CereBro, model call, package install, external
  write, storage migration, git action from CereBro, or Raven boundary changed.
- No worker was used because this was a single-file Security Gate UI
  read-gating slice.

### Front-End Steward Review
- Security Gate keeps target inspection, project link, and recent receipts
  primary.
- Source links are still available, but the source list reads only when the
  user asks for it.
- This keeps Spock's panel quieter and lighter by default.
...
`CEREBRO_MASTER_BUILD_PLAN.md`, and `CEREBRO_SESSION_HANDOFF.md`. Continue in
CereBro Prime mode. Start with a dirty-file read. Next best path: continue
broad-read reduction on Approval Queue or Hedwig, or add the next runtime
contract only if it closes a missing visible action in the Aang -> Workbench ->
Ledger loop.
...
### What Changed
- Removed the remaining polling timers from Terminal Lab's task, session,
  project, terminal receipt, and project receipt reads.
- Removed Project Lab's remaining overview, git status, and project receipt
  polling timers.
- Standardized both panels on 30 second stale local reads with focus/reconnect
  refetch disabled.
...
### Cleanliness Read
- Current slice: Project Lab and Terminal Lab idle polling cleanup.
- No backend behavior, DB schema, external write, model call, package install,
  command execution from CereBro, browser action from CereBro, storage
  migration, git action from CereBro, or Raven boundary changed.
- No worker was used because this was a two-file UI query-policy slice and the
  state stayed tightly coupled.

### Front-End Steward Review
- Terminal Lab still stays proposal-only.
- Project Lab still shows the project map, git read badge, receipt counts, and
  selected project inspector.
- The visible surfaces no longer keep rereading the same local rows on timers
  while idle.
...
remaining high-traffic surfaces for timer polling or move into the next runtime
receipt contract if the visible shell is calm enough.

## 2026-05-15 2008 EDT - Project Lab Detail Read Cache Pass

### What Changed
- Removed 10 second polling from the selected Project Lab detail read.
- Added a 30 second stale window and disabled focus/reconnect refetches for the
  selected project inspector.
- Kept mutation invalidation as the explicit refresh path when Project Lab draft
  notes change.
...
### Cleanliness Read
- Current slice: Project Lab selected-detail polling reduction.
- No backend behavior, DB schema, external write, model call, package install,
  command execution from CereBro, browser action from CereBro, storage
  migration, git action from CereBro, or Raven boundary changed.
- No worker was used because this was a single-file UI query-policy slice.

### Front-End Steward Review
- Project Lab cards and inspector behavior stay the same.
- The selected inspector no longer rereads approval rows, terminal rows,
  captures, reminders, messages, sources, action drafts, and git status every
  10 seconds while idle.
...
`CEREBRO_MASTER_BUILD_PLAN.md`, and `CEREBRO_SESSION_HANDOFF.md`. Continue in
CereBro Prime mode. Start with a dirty-file read. Next best path: inspect
whether Terminal Lab can use a lighter Project Lab/project label read, or move
into the next runtime receipt contract if the visible shell is calm enough.

## 2026-05-15 1957 EDT - Workbench Comparison Picker On Demand

### What Changed
- Gated Workbench `workbench.evidencePicker` behind the `Append Before/After
  Receipt` disclosure.
- Added a 30 second stale window and disabled focus/reconnect refetches for
  comparison candidate reads.
- Added visible copy inside the comparison drawer so the user sees that local
  comparison receipts read only when the drawer is open.
...
### Cleanliness Read
- Current slice: Workbench hidden comparison-picker read reduction.
- No backend behavior, DB schema, external write, model call, package install,
  command execution from CereBro, browser action from CereBro, storage
  migration, git action from CereBro, or Raven boundary changed.
...
### Front-End Steward Review
- Selecting a receipt still loads the receipt body.
- The larger comparison candidate list now waits until the user opens the
  append-comparison drawer.
...
`CEREBRO_MASTER_BUILD_PLAN.md`, and `CEREBRO_SESSION_HANDOFF.md`. Continue in
CereBro Prime mode. Start with a dirty-file read. Next best path: inspect
Project Lab overview weight for a DB-only summary split, or move into the next
runtime receipt contract if the visible shell is calm enough.

## 2026-05-15 1955 EDT - Design Review Receipt Picker On Demand

### What Changed
- Gated Design Review `workbench.evidencePicker` behind the linked receipt
  selector.
- Added a 30 second stale window and disabled focus/reconnect refetches for
  receipt picker reads.
- Moved the receipt-link read status next to the selector instead of the side
  rail.
...
### Cleanliness Read
- Current slice: Design Review hidden receipt-picker read reduction.
- No backend behavior, DB schema, external write, model call, package install,
  command execution from CereBro, browser action from CereBro, storage
  migration, git action from CereBro, or Raven boundary changed.
- No worker was used because this was a single-file UI query-gating slice.
...
### Front-End Steward Review
- The primary Design Review screen stays unchanged.
- Workbench receipt candidates now load only when the user opens the linked
  receipt selector.
...
`CEREBRO_MASTER_BUILD_PLAN.md`, and `CEREBRO_SESSION_HANDOFF.md`. Continue in
CereBro Prime mode. Start with a dirty-file read. Next best path: inspect
Workbench comparison picker reads or move into the next runtime receipt
contract if the visible shell is calm enough.
...
### What Changed
- Added `approvals.queue`, a compact read-only approval preview list.
- Added `approvals.detail`, a selected-decision detail read.
- Approval Queue now loads compact rows by default and loads full preflight plus
  Oak/Spock notes only for the selected decision.
- Kept `approvals.list` intact for compatibility with existing callers.
- Added targeted regression coverage for Hedwig and Terminal approval queue
  compact rows plus selected detail reads.
...
### Checks Run
- `pnpm -C app exec tsc --noEmit --pretty false` passed.
- `CEREBRO_DB_URL=file:/tmp/cerebro-approval-compact-test.db pnpm -C app exec vitest run server/cerebro-foundations.test.ts --pool=forks --fileParallelism=false -t "Approval|approval|Terminal Lab"` passed. 1 file. 6 tests run, 24 skipped.
- `pnpm -C app check` passed.
- `curl -I --max-time 5 http://localhost:3000/` returned `HTTP/1.1 200 OK`.
- `git diff --check` passed.

### Cleanliness Read
- Current slice: Approval Queue compact read-model split.
- No approval, rejection, command execution, browser action, external write,
  model call, package install, DB schema change, storage migration, git action
  from CereBro, or Raven boundary changed.
- No worker was used because the route, UI, and tests needed tight type
  alignment.

### Front-End Steward Review
- The default queue now reads like a decision list instead of a full evidence
  packet for every row.
- The proof still exists. It loads only when the user inspects a specific
  decision.
...
`CEREBRO_UX_SYSTEM.md`, `CEREBRO_BUILD_QUEUE.md`,
`CEREBRO_MASTER_BUILD_PLAN.md`, and `CEREBRO_SESSION_HANDOFF.md`. Continue in
CereBro Prime mode. Start with a dirty-file read. Next best path: inspect broad
Project Lab and Workbench reads for another safe compaction, or move into the
next runtime receipt contract if the visible shell is calm enough.
...
- Continued in CereBro Prime mode.
- Kept Raven, backend, and reference changes quarantined.
- Updated `HedwigInboxPanel` so capture input, local captures, triage proposal
  actions, proposal basics, reminder proposals, and message draft proposals
  stay visible.
- Moved capture rules behind `Capture Rules`.
- Moved preview Notion row fields behind `Proposed Notion Row`.
- Moved triage gates behind `Triage Gates`.
- Moved Notion capture database schema behind `Notion Capture Database`.
- Moved proposal review form, approval previews, and proposal gates behind
  closed details.
- Moved Slack shape, approval gates, and routing rules behind closed details.
- No backend behavior changed. No Slack read, Slack post, Notion write, browser
  action from CereBro, approval execution, command execution, project action,
  git action from the UI, Ollama command, install, pull, model call, vector
...
- `pnpm -C app check` passed.
- `curl -I --max-time 5 http://localhost:3002/` returned `HTTP/1.1 200 OK`.
- Playwright opened Capture and confirmed `Capture Rules`, `Notion Capture
  Database`, `Slack Shape`, `Approval Gates`, and `Routing Rules` details are
  present and closed.
- Playwright console check returned 0 errors.

### Cleanliness Read
- Current slice: Hedwig capture low-machinery pass.
- Quarantine: `AGENTS.md`, `CEREBRO_EXTERNAL_REFERENCE_INTEGRATION_PLAN.md`,
  Raven ADR, the existing Raven entry change in `app/client/src/pages/Home.tsx`,
  and dirty backend/Raven files remain unstaged.
- Generated/local: `.playwright-cli/` regenerated during browser proof and
...
### Front-End Steward Review
- Hedwig now reads as capture first.
- Proposal-only safety still shows, but schema and approval machinery no longer
  compete with the intake flow.
- This keeps Hedwig useful as the fast capture lane without asking the user to
  understand Slack, Notion, and approval internals by default.

### Completion Read
- Overall: 49%.
- Foundation/docs/planning: 92%.
- Frontend visible loop: 80%.
- Backend/runtime: 30%.
- Knowledge/storage/source: 36%.
- Creative/freelance/watch: 10%.
- Confidence: medium.
...
worker frees up. Low-machinery pass is mostly complete across clean component
surfaces. Next best path: either resolve the `Home.tsx` Raven entry conflict so
Ledger/Home shell polish can continue, or switch to backend receipt contracts
after classifying the dirty backend/Raven batch.
...
- Continued in CereBro Prime mode.
- Kept Raven, backend, and reference changes quarantined.
- Updated `ModelToolsPanel` so registry, selected capability detail, creative
  lane cards, and route preview stay visible.
- Moved Ollama install-status internals behind `Install Status Check`.
- Moved first-batch/stretch model lists behind `Model Batches`.
- Moved blocked model-start list behind `Do Not Start With`.
- Moved local capability proposal form behind `Propose`.
- Moved local eval note form and eval history behind `Eval Note`.
- Moved Model Tools policy gates behind `Gates`.
- No backend behavior changed. No provider call, model call, Ollama command,
  install, pull, source fetch, browser action, approval execution, command
  execution, external write, vector index, or storage migration ran.
...
- `pnpm -C app check` passed.
- `curl -I --max-time 5 http://localhost:3002/` returned `HTTP/1.1 200 OK`.
- Playwright opened Basement, opened Models, and confirmed `Install Status
  Check`, `Model Batches`, `Do Not Start With`, `Propose`, `Eval Note`, and
  `Gates` are collapsed while the registry and route preview stay visible.
- Playwright console check returned 0 errors.
- `git diff --check -- app/client/src/components/ModelToolsPanel.tsx` passed.

### Cleanliness Read
- Current slice: Model Tools low-machinery pass.
- Quarantine: `AGENTS.md`, `CEREBRO_EXTERNAL_REFERENCE_INTEGRATION_PLAN.md`,
  Raven ADR, the existing Raven entry change in `app/client/src/pages/Home.tsx`,
  and dirty backend/Raven files remain unstaged.
- Generated/local: `.playwright-cli/` regenerated during browser proof and
...
### Front-End Steward Review
- Model Tools now reads as capability map first.
- Local-write forms and setup internals are still there, but they no longer
  turn Basement into a wall of controls.
- This keeps the machine powerful without making the user operate the machine.

### Completion Read
- Overall: 47%.
- Foundation/docs/planning: 92%.
- Frontend visible loop: 78%.
- Backend/runtime: 30%.
- Knowledge/storage/source: 36%.
- Creative/freelance/watch: 10%.
- Confidence: medium.
...
as `HedwigInboxPanel`, `PiccoloPanel`, `ArtifactsPanel`, or `MemoryPanel`.
Avoid `Home.tsx` until Raven entry work is resolved. Switch to backend receipt
contracts only after the Raven backend batch is resolved.

## 2026-05-14 2132 EDT - Research Source Detail Pass

### What Changed
- Continued in CereBro Prime mode.
- Kept Raven, backend, and reference changes quarantined.
- Updated `SurferSourcesPanel` so saved-source cards keep title, type, trust,
  summary, and source display visible.
- Moved per-card approval and scrub notes behind `Source Rules`.
- Moved the right-rail browser ladder behind `Browser Ladder`.
- Moved the right-rail source policy and preview gates behind `Policy`.
- No backend behavior changed. No source fetch, browser action, approval
  execution, command execution, project action, git action from the UI, Ollama
  command, install, pull, model call, vector index, external write, or storage
  migration ran.
...
- `pnpm -C app check` passed.
- `curl -I --max-time 5 http://localhost:3002/` returned `HTTP/1.1 200 OK`.
- Playwright opened Workshop, opened Research, and confirmed saved-source
  summary remains visible while `Source Rules`, `Browser Ladder`, and `Policy`
  are collapsed.
- Playwright console check returned 0 errors.
- `git diff --check -- app/client/src/components/SurferSourcesPanel.tsx` passed.

### Cleanliness Read
- Current slice: Research/source low-machinery pass.
- Quarantine: `AGENTS.md`, `CEREBRO_EXTERNAL_REFERENCE_INTEGRATION_PLAN.md`,
  Raven ADR, the existing Raven entry change in `app/client/src/pages/Home.tsx`,
  and dirty backend/Raven files remain unstaged.
- Generated/local: `.playwright-cli/` regenerated during browser proof and
...
### Front-End Steward Review
- Research now reads as source library first.
- Trust, summary, and source identity stay visible.
- Browser ladder and policy remain reachable, but they no longer compete with
  the research object itself.

### Completion Read
- Overall: 47%.
- Foundation/docs/planning: 92%.
- Frontend visible loop: 77%.
- Backend/runtime: 30%.
- Knowledge/storage/source: 36%.
- Creative/freelance/watch: 10%.
- Confidence: medium.
...
files. Keep Raven implementation quarantined because Raven is being built in a
separate chat. Worker slots may already be full, so use local slices unless a
worker frees up. Next safe frontend slice: inspect `ModelToolsPanel` for
default-visible machinery, especially Ollama setup, local model lanes, creative
lanes, proposal/eval forms, route preview, and gates. Avoid `Home.tsx` until
Raven entry work is resolved. Switch to backend receipt contracts only after the
Raven backend batch is resolved.

## 2026-05-14 2113 EDT - Security Gate Receipt Detail Pass

### What Changed
- Continued in CereBro Prime mode.
- Kept Raven, backend, and reference changes quarantined.
- Updated `SecurityGatePanel` so inspected receipt output keeps target, risk,
  project link, and source link visible.
- Moved findings, blocked actions, allowed actions, checks, and browser profile
  policy behind `Receipt Details`.
- Moved scanner adapter details behind `Scanner Plan`.
...
- `pnpm -C app check` passed.
- `curl -I --max-time 5 http://localhost:3002/` returned `HTTP/1.1 200 OK`.
- Playwright opened Security Gate, inspected `https://example.com`, and
  confirmed target/risk/link fields stay visible while `Receipt Details` and
  `Scanner Plan` are collapsed.
- Playwright console check returned 0 errors.
- `git diff --check -- app/client/src/components/SecurityGatePanel.tsx` passed.

### Cleanliness Read
- Current slice: Security Gate receipt-detail low-machinery pass.
- Quarantine: `AGENTS.md`, `CEREBRO_EXTERNAL_REFERENCE_INTEGRATION_PLAN.md`,
  Raven ADR, the existing Raven entry change in `app/client/src/pages/Home.tsx`,
  and dirty backend/Raven files remain unstaged.
- Generated/local: `.playwright-cli/` regenerated during browser proof and
...
### Front-End Steward Review
- Security Gate now reads as a decision receipt first.
- Scanner internals and browser-policy details remain available, but they do
  not occupy the default result view.
- This supports the product rule: visible gate, hidden machinery, no accidental
  action.

### Completion Read
- Overall: 46%.
- Foundation/docs/planning: 92%.
- Frontend visible loop: 76%.
- Backend/runtime: 30%.
- Knowledge/storage/source: 36%.
- Creative/freelance/watch: 10%.
- Confidence: medium.
...
files. Keep Raven implementation quarantined because Raven is being built in a
separate chat. Worker slots may already be full, so use local slices unless a
worker frees up. Next safe frontend slice: inspect `SurferSourcesPanel` or
`ModelToolsPanel` for remaining default-visible machinery. Avoid `Home.tsx`
until Raven entry work is resolved. Switch to backend receipt contracts only
after the Raven backend batch is resolved.
...
## 2026-05-16 0516 EDT - Runtime Workbench project-focus contract
...
- No worker used. This was a narrow backend/runtime contract with one client
  consumer path.

What changed:

- `runtime.previewRoute` Workbench drafts now include a formal `projectFocus`
  object.
- The focus object carries project slug, name, path, null project id,
  preview-only resolution, and autosave false.
- Ask Aang's route preview Workbench action now forwards project name, path,
  and `projectFocus` into the staged Workbench draft.
- Workbench draft loading can now resolve a preview-only project focus by id,
  name, or local path after project options load.
- The visible Workbench chain now keeps the project context and shows
  `Project Lab reads CereBro` before any local receipt is saved.
...
- Red first:
  `CEREBRO_DB_URL=file:/tmp/cerebro-runtime-workbench-focus-red.db pnpm -C app exec vitest run server/runtime.routeReceipt.test.ts --pool=forks --minWorkers=1 --maxWorkers=1`
  failed because `workbenchReceiptDraft.projectFocus` was undefined.
- Green after patch:
  `CEREBRO_DB_URL=file:/tmp/cerebro-runtime-workbench-focus-green.db pnpm -C app exec vitest run server/runtime.routeReceipt.test.ts --pool=forks --minWorkers=1 --maxWorkers=1`
  passed, 11 tests.
- `pnpm -C app check` passed.
- Browser proof opened `http://localhost:3000`, set Ask Aang to Build,
  previewed `keep building CereBro front end`, clicked the route preview
  Workbench action, and confirmed Workbench showed
  `Project Lab reads CereBro` plus the staged local draft.

Known risks:

- Workbench project focus still depends on Project Intelligence returning the
  matching project profile. This is fine for the locked V1 project map.
- The staged route preview remains review-only. It does not save a Workbench
  receipt, route record, task, command, model call, browser action, memory
  write, Notion write, Slack write, or external write.
...
## 2026-05-16 0704 EDT - Terminal Lab observation next steps
...
- No worker used. This was a narrow follow-up to the named Terminal Lab risk.

What changed:

- Added `terminalLabObservationActionCopy` to the Terminal Lab copy model.
- The observation drawer now says `Observation Next Steps` instead of
  `Observation Actions`.
- Action groups now read `Status`, `Approval`, `Connect`, and `Teach +
  Receipt` instead of `Review`, `Gate`, `Link`, and `Output`.
- Buttons now use clearer visible next-action labels: `Approval Read`,
  `Security Gate`, `Link Selected`, `Make Task`, `Learning Note`, `Aang Teach`,
  `Receipt Body`, and `Ledger`.
- The copy test now blocks the old `Output` group and the old `Gate` group
  label while preserving the real `Security Gate` destination.
...
- Red first:
  `pnpm -C app exec vitest run server/terminalLabCopyModel.test.ts`
  failed because the first guard banned the real `Security Gate` destination.
- Green after tightening the assertion:
  `pnpm -C app exec vitest run server/terminalLabCopyModel.test.ts` passed,
  3 tests.
- `git diff --check -- app/client/src/components/TerminalLabPanel.tsx
  app/client/src/lib/terminalLabCopyModel.ts
...
- `pnpm -C app check` passed.
- Browser proof opened `http://localhost:3000/`, opened Workshop, opened
  Terminal Lab, opened the first observation drawer, and confirmed
  `Observation Next Steps`, `Status`, `Approval`, `Connect`, `Teach + Receipt`,
  `Approval Read`, `Link Selected`, `Make Task`, `Learning Note`, `Aang Teach`,
  `Receipt Body`, and `Ledger`.
...
- On path because Terminal Lab is the approved code-learning and command
  teaching lane.
- Creative UI/UX stayed inside the lane: same drawer, no behavior expansion,
  less machinery, clearer next steps.
- No new surface, agent, plugin, model, external source, Raven path, GitHub
  implementation, or command execution feature was added.

Known risks:

- Terminal Lab is now more stable. Next best pass should move to Workbench
  visual proof/body clarity, unless a final Terminal Lab density issue appears
  in browser review.
